...
//设置滑动页面的标题
wui.viewpager.setTitles(["引流配置", "引流术语", "教程"]);
//让滑动页面和标签栏联动
wui.tabs.setupWithViewPager(ui.viewpager);

//让工具栏左上角可以打开侧拉菜单
wui.toolbar.setupWithDrawer(ui.drawer);


// wui.menu.on("item_click", item => {
//     switch(item.title){
//         case "退出":
//             ui.finish();
//             break;
...
//
function savehuashu(){
    let hs=wui.in_chatcontent.text()
    storage.put("yinliucontent",hs)
}
...
wui.in_spm.setText(商品内容)
wui.in_chatcontent.setText(引流内容)
wui.in_onelivetime.setText(""+onelivetime)
wui.in_livepersonmin.setText(""+livepersonmin)
wui.bt_run.on("click",function(v){
    run()
})
wui.web.loadUrl("https://hao.360.com/?src=lm&ls=n7303c05692")

wui.bt_save1.on("click",function(v){
    //
    saveconfig1()
})

wui.bt_save2.on("click",function(v){
    //保存话术
    savehuashu()
})
