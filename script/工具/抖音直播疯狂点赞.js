...
var clickX=0; //点击X
var clickY=0; //点击Y
var clicktime=0; //记录实时的点击次数
var clicksumtime=0; //记录点击次数
var interval=1000// 点击位置
var clickthread=null;
var keeptoedge=false
...
function randint(min,max){
    return random(0,max-min)+min
}
function sclick2(px,py){
    // press(clickX+randint(-10,10),clickY+randint(-10,10),20)
    // press(clickX+randint(-10,10),clickY+randint(-10,10),20)

    gestures([0, 50, [px, py], [px+randint(-2,2), py+randint(-3,3)]],
    [2, 45, [px+randint(-5,5), py+randint(-5,5)], [px+randint(-5,5), py+randint(-5,5)]],
