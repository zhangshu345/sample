...
function showLoginUI(){
    ...
    ui.scan.on("click",(v) =>{
        if(v.getText()=="录入"){
            events.observerScan("录入二维码",111,true)
        } else{
...
function addscanimg(txt){
    // let im = new android.widget.ImageView(context)
    // im.setLayoutParams(new android.view.ViewGroup.LayoutParams(250,250));
    

}

threads.start(function(){
    events.onScan(function(txt){
        toastLog(txt)
       //  ui.run(addscanimg(txt))
    })
})
