...
const appname = '趣铃声';
const package = 'com.zheyun.bumblebee';
...
function autoRedraw() {
    ...
    if (o) {
        func.clickObject(o);
        func.sleep(10000, '等待中', "textMatches('提现').visibleToUser().exists() || descMatches('提现').visibleToUser().exists()");
        func.sleep(2000)    
        ...
        if (o) {
            ...
            var money = 0;
            o = packageName(package).textMatches('约[0-9.]*元').visibleToUser().findOne(5000)
            if (o)
            ...
            func.toast('金额:' + o.text(), 1)
            if (money >= 0.3) {
                var btn = packageName(package).text('立即提现').visibleToUser().findOne(5000) || packageName(package).desc('立即提现').visibleToUser().findOnce();
                o = packageName(package).textMatches('[0-9.]*').visibleToUser().filter(function (w) {
                    var r = Number(w.text()) || 0;
                    return r > 0 && r <= money;
                }).findOne(10000);  //这10秒是等待金额加载出来
                var btns = packageName(package).textMatches('[0-9.]*').visibleToUser().filter(function (w) {
                    var r = Number(w.text()) || 0;
                    return r > 0 && r <= money;
                }).find();
                if (btns.length == 0)
                    btns = packageName(package).descMatches('[0-9.]*').visibleToUser().filter(function (w) {
                        var r = Number(w.desc()) || 0;
                        return r > 0 && r <= money;
                    }).find();
                    ...
                    if (btn){
                        ...
                        if (!text('立即提现').visibleToUser().exists() && !desc('立即提现').visibleToUser().exists()){
                            if (textMatches('.*更懂你').visibleToUser().exists() || descMatches('.*更懂你').visibleToUser().exists()){
                                ...
                                toast("提现完成")
                                break
                            }else if(text('去绑定').visibleToUser().exists() && desc('去绑定').visibleToUser().exists()){
...
function sign() {
    try {
        ...
        if (o){
            func.clickObject(o)
            func.sleep(10000, '等待中', "textMatches('瓜分金币').visibleToUser().exists() || descMatches('瓜分金币').visibleToUser().exists()");
            o = textMatches('瓜分金币').visibleToUser().findOnce();
            if (o) {
                func.clickObject(o);
                func.sleep(10000, '等待中', "textMatches('瓜分金币').filter(function(w){return w.bounds().bottom < device.height * 0.3;}).visibleToUser().exists()");
                func.sleep(4000);
                if (text('瓜分金币').visibleToUser().exists()) {
                    ...
                    while (n-- > 0) {
                        ...
                        if (o) {
                            func.clickObject(o);
                            func.sleep(2000);
                            func.sleep(60000, "瓜分金币中", "idMatches('" + package + ":id/.*close.*').visibleToUser().exists() || packageName('" + package + "').textMatches('" + keys + "').filter(function(w){if (w.text()=='点击下载' && w.bounds().bottom > device.height*0.8){return false}else{return true}}).visibleToUser().exists()");
                            closeDialog();
                            func.sleep(3000);
                            closeDialog();
                            func.sleep(2000);
