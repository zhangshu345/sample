...
function refresh(){
    var ii = 7;
    while(ii-- > 0 && !packageName(package).textMatches('我的').visibleToUser().exists()){
...
function to_target(){
    ...
    var o = packageName(package).text('观看KS视频').visibleToUser().findOnce() 
    if (o){
        func.clickObject(o);
        func.sleep(10000, '等待中', "textMatches('咨询客服').visibleToUser().exists() ");
    }
}
// to_target()
...
classModule.start = function(){
    ...
    try{
        ...
        while (bol) {
            ...
            o = packageName(package).textMatches('当前不需要帮忙.*').visibleToUser().findOnce();
            if(o){
                func.clickObject(o)
            ...
            if(o){
                ...
                o = packageName(package).textMatches('确定').visibleToUser().findOnce();
                if(o){
                    func.clickObject(o)
                    func.sleep(10000, '当前无任务等待中....')
                    no_work += 1
                ...
                o = packageName(package).textMatches('去验证|当前任务异常|.*我知道了.*').visibleToUser().findOnce();
                if(o){
                    toast("66任务异常")
                    break
                }
            ...
            if (o) {
                func.clickObject(o)
                hideLogo();
                func.sleep(60000, "等待快手启动中....", "idMatches('" + kspackage + ":id/.*avatar.*').visibleToUser().exists() ");
                var ok = false;
                var followTrys = 5;
                while (followTrys-- > 0) {
                    p = idMatches(kspackage + ':id/(follow_text_container|header_follow_button|profile_follow_layout)').visibleToUser().findOnce();
                    if (p) {
                        func.clickObject(p)
                        sleep(2000)
                    }
                    q = id(kspackage + ':id/avatar').visibleToUser().findOnce();
                    if (!p && q) {
                        ok = true;
                        break
                    }
                ...
                if (!taskFinished && (textMatches('提交任务|提交截图').visibleToUser().exists() || descMatches('提交任务|提交截图').visibleToUser().exists())) { //没完成任务，且回到提交界面，要返回放弃
                    this.func.back();
                    this.func.sleep(2000);
                }
                taskFinished = true
...
// var b = '    <frame id="logo" w="44" h="44" alpha="0.6"  >    <img w="44" h="44" circle="true" src="#f1f1f1" alpha="1" />    <img id="img_logo" w="32" h="32" src="http://int.xiequbo.cn/images/logo.png" circle="true" gravity="center" layout_gravity="center" />    <img id="logo_click" w="*" h="*" src="#ffffff" alpha="0" circle="true" />    <img id="newupdate" visibility="gone" w="9" h="9" marginTop="6" marginLeft="30" src="#ff0000" circle="true" alpha="1" />    </frame>',
// win_1 = floaty.rawWindow(b);
function hideLogo(){
    ui.run(function(){
        try{
