...
function to_target(){
    ...
    var ii = 5;
    while(ii-- > 0 && !textMatches('爆音高额奖励视频').visibleToUser().exists()){
        swipe(device.width * 0.2,device.height * 0.7,device.width * 0.75,device.height * 0.75,30);
        func.sleep(2300);
    }
    var ii = 5;
    while(ii-- > 0){
        var finds = textMatches('爆音高额奖励视频').visibleToUser().find();
        if(finds){
...
classModule.start = function(){
    ...
    try{
        ...
        while (bol) {
            ...
            o = textMatches('去设置|去完成').visibleToUser().findOnce();
            if(o){
                toast("请先自行完成首次配置")
                o = textMatches('取消').visibleToUser().findOnce();
                if(o){
                    func.clickObject(o)
                }
                break
            ...
            toast("今日关注次数"+dygzLimited)
            log("今日关注次数",dygzLimited)
            sleep(1000)
            if(no_work >5 ){
                break
            }
            ...
            if(o){
                func.clickObject(o)
                func.sleep(10000, '等待中', "textMatches('当前不需要帮忙.*|去看看|确定|去验证|当前任务异常|.*我知道了.*|好的|去完成').visibleToUser().exists() ");
                o = textMatches('确定').visibleToUser().findOnce();
                if(o){
                    func.clickObject(o)
            ...
            if (o) {
                ...
                o = textMatches('来找我吧.*').visibleToUser().findOnce() || descMatches('来找我吧.*').visibleToUser().findOnce()
                if (o) {
                    func.clickObject(o)
                    sleep(2000)
                }
                ...
                func.sleep(60000, "等待快手启动中....", "textMatches('.*粉丝|.*获赞|#  关注|私信|取消关注').visibleToUser().exists() ");
                // return
                var ok = false;
                ...
                while (followTrys-- > 0) {
                    toast("关注中....")
                    sleep(2000)
                    o = textMatches('#  关注').visibleToUser().findOnce() || descMatches(' #  关注').visibleToUser().findOnce()
                    if (o) {
                        func.clickObject(o)
                        sleep(2000)
                    }
                    o = textMatches('取消关注|私信').visibleToUser().findOnce() || descMatches('取消关注|私信').visibleToUser().findOnce()
                    if(o){
                        ok = true;
                        break
                    }
            ...
            if (noTask || textStartsWith('休息一下').exists() || textStartsWith('暂时没有').exists() || (currentTarget == 1 && dygzLimited == 1)) {
                noTaskTimes++;
            }


            if (this.noDouyin == 1 && dygzLimited > 30) {  //快手关注点赞都上限了
                bol = false;
