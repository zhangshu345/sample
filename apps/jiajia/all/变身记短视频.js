const appname = '变身记短视频';
const package = 'com.video.bsj';
    ...
    try{
        if (pickDate <= Date.now()){
            ...
            if (o){
                ...
                if (packs.length == 0){
                    o = idMatches(package + ':id/(red_view|red_view_content)').visibleToUser().findOnce();
                    func.sleep(2000);
                    var packs = id(package + ':id/item_content').visibleToUser().filter(function(w){
                        return w.childCount() == 0;
                    }).find()
                ...
                for(var i = packs.length - 2; i >= 0 ; i--){
                    ...
                    /*while(!id(package + ':id/tiny_red_pack_item').visibleToUser().exists() && !id(package + ':id/red_view').visibleToUser().exists()){
