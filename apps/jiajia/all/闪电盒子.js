...
classModule.start = function(){
    ...
    try{
        ...
        while(bol){
            ...
            if(!packageName(package).textMatches('头条资讯|关注|逛一逛即得闪电币奖励|我的喜欢').visibleToUser().exists() && !idMatches(package + ':id/(logo|top_text|icon_finish_layout)').visibleToUser().exists()){
            ...
            if (intext == 2 && !idMatches(package + ':id/(iv_like|layout_like)').visibleToUser().exists()){  //当前是小视频，但没有进入小视频，要重新进入
                var ii = 5;
            ...
            else if (intext == 3 && !packageName(package).text('逛一逛即得闪电币奖励').visibleToUser().exists()){ 
                var ii = 5;
                while(!packageName(package).text('首页').visibleToUser().exists() && ii-- > 0){
                    func.back();
                    func.sleep(2500);
            ...
            else if (new Date().getTime() - prevRedPacketDate.getTime() > 5 * 60 * 1000){  //10分钟后再收一次红包
                prevRedPacketDate = new Date();
                pickRedPacket();    //收首页的红包
            }
            ...
            if (intext == 2){   //当前是小视频，但又没进入小视频
                ...
                var child = id(package + ':id/title').visibleToUser().filter(
                    function(w){
                        var b = w.bounds();
                        var r = b.width() > 0 && b.bottom > b.top && b.bottom < device.height - 280 && w.text().length > 8;
                ...
                if (child){   
                    ...
                    if (readeds.join().indexOf(txt) < 0){   //不在本次历史，不在总历史
                        ...
                        while(isopen > 0){
                            ...
                            var act = currentActivity(); 
                            if (packageName(package).text('收藏').visibleToUser().exists() || idMatches('.*(viedoContainer|vp_v_bg|titleS|news-title|video_title)').exists()){
...
function autoRedraw(){
    ...
    if (o){
        ...
        if (o){
            ...
            if (money >= 1){
                o = packageName(package).text('提现').visibleToUser().findOnce() || id(package + ':id/action_btn_1').visibleToUser().findOnce();
                func.clickObject(o)
                func.sleep(2000);
...
function tvCoin(){
    try{
        var ii = 10;
        while (ii-- > 0){
            var o = idMatches(package + ':id/tv_coin.*').visibleToUser().findOnce() || packageName(package).text('领取奖励').visibleToUser().findOnce();
            if (o){
                func.clickObject(o);
                func.sleep(2000);
                o = idMatches(package +':id/.*close.*').findOnce() //弹出有关闭窗口
                if (o){
                    func.clickObject(o);
                    func.sleep(1000);
                }
        ...
        while(ii-- > 0){
            ...
            if (packageName(package).text('查看更多可领取红包》').visibleToUser().exists()){
                ...
                if (packs.length > 1){
                    ...
                    o = textMatches('看视频最高可得[0-9 ]*闪电币|观看视频奖励翻倍').visibleToUser().findOnce();
                    if (o){
                        func.clickObject(o);
                        func.sleep(2000);
                        func.sleep(60000,"看视频最得闪电币中","idMatches('" + package + ":id/.*close.*').visibleToUser().exists() || packageName('" + package + "').textMatches('点击下载|点击打开|头条资讯|查看更多可领取红包》').visibleToUser().exists()")
                        closeDialog();
                    }
                    else{
                        func.back();
...
function refresh(){
    try{
        var tt = id(package +':id/tab_text').findOnce();
        if (tt){
            func.clickObject(tt);
            func.sleep(50)
            func.clickObject(tt);
...
function sign(){
    ...
    if (o){
        ...
        o = packageName(package).text('立即签到').visibleToUser().findOnce();
        if (o){
            func.clickObject(o);
            func.sleep(1000);
