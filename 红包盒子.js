...
function essay() {
    ...
    have_read = 0
    while (true) {
        first_video_close = id('three_title').boundsInside(0, h * 0.2, w, h * 0.8).find(500)
...
function 启动线程(type, sign) {
    threads.start(function(){
        star_ing = text("立即开始").findOne(5000)
        if(star_ing){
            star_ing.click()
            return
        }
    ...
    // find_img_result = find_img(images.read(filePath))
    hdjksahjkdsa = images
    var templ = hdjksahjkdsa.read(filePath);
    // var p=images.findImage(captureScreen(),find_img_result)
    log(8)
    toast("开始检测的")
